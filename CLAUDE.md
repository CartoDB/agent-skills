...
- `make package` builds per-skill zips into `dist/` (gitignored) for web-upload hosts such as Claude.ai and Gemini Enterprise. Don't commit zips; the `release` workflow attaches them to a GitHub Release when a `v*` tag is pushed.
