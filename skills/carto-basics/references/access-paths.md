...
| Run a saved Workflow as an analytical tool | MCP (`run_workflow`, or a published Workflow tool) | CLI | covered ad-hoc by host's tool-list |
...
> **Tool availability depends on how the MCP server was authenticated.** A user signed in over OAuth reaches the full surface over MCP, including map/workflow authoring and admin. A user connected with an API Access Token reaches a read-and-discovery subset (explore, query, run workflows, search the Data Observatory) — authoring and admin tools aren't offered on that token session, so either reconnect the MCP server over OAuth or fall back to the CLI for those tasks.
