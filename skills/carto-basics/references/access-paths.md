...
CLI-action skills (`carto-explore-datawarehouse`, `carto-query-datawarehouse`, `carto-create-workflow`, etc.) are CLI-only today. As they become mode-aware (per-skill MCP/CLI routing), this table narrows. Until then, it's the routing source of truth.
