...
6. **Basemap** per [`migrate-maps/basemap-mapping.md`](migrate-maps/basemap-mapping.md): map the source `baseMap.title` (or `baseMapLayers[].url` pattern) to a CARTO basemap; write **both** `basemapConfig` (`{ styleId }` — there is no `type` field) and `mapStyle.styleType`, with the same id in each. Honor an explicit `Basemap override:` field on the manifest entry if present.
...
7. **App map controls → Builder `mapSettings`**: some ArcGIS app controls (Basemap switcher, Search, Measurement, Locate) have a Builder mapSettings flag; Layer list, Legend and Bookmarks don't, and are recorded as `Notes: app-control-skipped: <name>`. Read the live shape with `carto maps schema mapsettings`; apply the per-control mapping table from `app-absorption.md`. `mapSettings` accepts unknown keys silently, so an invented flag name toggles nothing — never guess one.
...
1. **Resolve dataset metadata for every layer** per [`migrate-maps/dataset-config.md`](migrate-maps/dataset-config.md). For each unique source FQN referenced by the map's layers:
   ...
   - Build each `dataset` entry — in the bundle's **top-level `datasets[]`**, a sibling of `title` and `keplerMapConfig`, not inside `visState` — with:
     - `$ref` (a **symbolic name** you choose, e.g. `"bus_stops"` — not a UUID; each layer points at it as `"dataId": "$ref:bus_stops"`), `type: "table"` (or `"query"` if Arcade derived fields are needed)
     - `source` (FQN), `connectionId` (UUID). **Not `connectionName`** — it is server-derived and not writable.
     - `geoColumn`, `columns` (**non-null string array including `geoColumn`**)
     - `format: "tilejson"` (optional — derived from `type` when omitted), `label`
     - **`color`: a hex string** like `"#7F3C8D"` (Builder's default purple). **NOT** an int array `[r, g, b]` and **NOT** a Postgres array literal — the backing column is `text`; int arrays get coerced to `text[]` form that Builder can't parse on read. Cycle through a small palette across datasets if there are several (e.g. `#7F3C8D` / `#11A579` / `#3969AC` / `#F2B701` / `#E73F74` / `#80BA5A` / `#E68310` / `#008695` by index mod 8).
     - `aggregationExp: null`, `aggregationResLevel: null`, `spatialIndex: null`, `queryTemplate: null`, `queryParameters: null` for a plain tileset dataset
...
2. **Initialize top-level filter state**: set `keplerMapConfig.config.filters` to an **object keyed by dataset id**, with empty `{}` values per dataset. **NOT** `[]` (the legacy kepler array shape — Builder's loader crashes on it; verified). See [`migrate-maps/dataset-config.md`](migrate-maps/dataset-config.md) "Top-level filter state".
3. **Popup settings — omit when source has no popups**: if `popupInfo` is null/absent on every source layer, **do not emit `keplerMapConfig.config.popupSettings` at all** (not even `{layers: {}}` or `{layers: []}`), and set `visState.interactionConfig.tooltip.enabled: false` so Builder's legacy-tooltip conversion doesn't manufacture the popups you just left out. Migration faithfully reproduces source behavior. Only emit `popupSettings` when at least one source layer has a populated `popupInfo`, then follow [`migrate-maps/popup-mapping.md`](migrate-maps/popup-mapping.md).
4. **Normalize per-layer style defaults** per [`migrate-maps/renderer-mapping.md`](migrate-maps/renderer-mapping.md) "Required non-null layer-config fields". After translating each layer's renderer, ensure `config.color` (the fill, an RGB triplet — **not** `visConfig.fillColor`, which is not a field) and `visConfig.strokeColor` / `opacity` / `radius` / `thickness` are set, and that `visualChannels` exists on the layer as a sibling of `config` (`{}` when nothing is data-driven).
...
5. **Apply kepler boilerplate defaults** per [`migrate-maps/mapconfig-defaults.md`](migrate-maps/mapconfig-defaults.md). Builder's loader requires the following to be present with specific shapes even on minimal maps:
   ...
   - **`visState.interactionConfig`**: `{brush: {enabled: false, size: 0.5}, coordinate: {enabled: false}, geocoder: {enabled: false}, tooltip: {compareMode: false, compareType: "absolute", enabled: <true only when the map emits popupSettings>}}`. Not `null`, not `{}`; `tooltip` is mandatory whenever the object is present.
...
6. Compose the full `keplerMapConfig` JSON. **Always fetch live schema** with `carto maps schema <section>` and validate fragment shapes inline; never hardcode kepler shapes (the schema is the source of truth — see [`migrate-maps/lessons.md`](migrate-maps/lessons.md) "Never hardcode kepler schema"). The sections that exist are `bundle`, `dataset`, `privacy`, `agent`, `kepler`, `mapstate`, `mapstyle`, `mapsettings`, `layer`, `layers`, `widgets`, `sqlparameters`, `popupsettings`, `uistate`, `legendsettings`, `layergrouping`, `layer.tileset`, `layer.h3`, `layer.quadbin`, `layer.heatmaptile`, `layer.clustertile`, `layer.raster`, `enums`, `example` and `palettes` — anything else returns `Unknown section`. For everything the schema marks optional but Builder runtime requires, see [`migrate-maps/mapconfig-defaults.md`](migrate-maps/mapconfig-defaults.md) (config-level) and [`migrate-maps/renderer-mapping.md`](migrate-maps/renderer-mapping.md) (layer-level) — start from those defaults and modify.
7. **Title**:
