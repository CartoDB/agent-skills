...
**Legend required after every render.** The `view_map` renderer does NOT show an auto-legend. Immediately after invoking `view_map`, emit a compact HTML legend covering every data-driven layer. Transport is host-agnostic: if a widget tool is available (e.g., `show_widget` in Claude.ai / Claude Desktop), call it with the HTML; otherwise emit the HTML in your chat reply (every MCP App host that renders the map also renders inline-styled HTML in markdown). The full template, style rules, and per-helper variants (`colorBins` swatches, `colorContinuous` gradient bar, `colorCategories` per-category, raster ternary-bucket + nodata) live in the `view_map` tool description's LEGEND section — read it once and follow it. Applies to `view_map` ONLY; `load_builder_map` has its own Builder-native legend.
