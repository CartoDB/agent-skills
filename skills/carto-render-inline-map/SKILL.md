...
**No auto-legend.** The renderer does NOT show an inline legend, so after invoking `view_map` your chat reply must describe the encoding for each data-driven layer: layer name, the column being styled (`attr`), the palette name, and the bucket thresholds (for `colorBins`) / category mapping (for `colorCategories`) / domain endpoints (for `colorContinuous`). For raster layers, describe the band, the value range each color represents, and the nodata sentinel. Multi-layer maps need one short paragraph per data-driven layer. Without this, the user sees colors with no key.
