...
- **Mixing controlled `viewState` with user pan/zoom** is the top way to break a storymap. `onViewStateChange` fires on *every frame* of a flight, so the naive `setViewState(viewState)` feeds the in-flight interpolator back into controlled state and cancels it — the camera stays pinned on step 0 while cards, dots, and legends advance, presenting as "my tiles/layers are broken". The two guards (gate on `interactionState.is*`; strip the `transition*` props before storing) are both in the snippet above. Also strip `width`/`height` if you spread the incoming object, or a stale `300×150` pins the viewport permanently (blank map, healthy tilejson).
- **`IntersectionObserver` mid-transition.** Throttle the callback (≥250 ms) or you'll fire `setStepIdx` mid-flight and stutter.
