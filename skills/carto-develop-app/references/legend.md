...
So `[10_000, 50_000, 100_000, 500_000]` (4 edges) needs **5 swatches** in the legend. `cartocolor` keys sequential and diverging palettes 2–7, and qualitative ones 2–11. Pick the smallest stop count ≥ your bucket count.
...
`fetchMap` returns the palette and domain on each layer, under `mapInfo.layers[i].scales` — one entry per styled visual channel (`fillColor`, `lineColor`, `pointRadius`, …), each carrying `{ type, field, domain, range }`. Read those instead of re-deriving the bins. Presentation options the map author set in Builder — which layers appear in the legend, entry labels, sort order — arrive separately at `mapInfo.legendSettings`, keyed by layer id. See the `fetchmap` example in [deck.gl-examples](https://github.com/CartoDB/deck.gl-examples/tree/master/fetchmap).
...
- **Palette stop count** — `cartocolor[palette]` is keyed by stop count: `[2]`…`[7]` for sequential and diverging palettes, `[2]`…`[11]` for qualitative ones. Pick the smallest one `≥ bucket count`. Past a palette's largest key, redesign the bins (or pre-interpolate yourself) — most readable numeric legends top out at 7 anyway.
- **Qualitative palettes return one extra colour.** `cartocolor[qualitative][n]` is `n + 1` entries — the last is a grey "other" bucket. Slice it off unless you want it.
