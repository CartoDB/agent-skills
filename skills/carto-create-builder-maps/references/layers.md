...
- **The pattern is drawn into the legend swatch**, so a patterned layer reads the same in the legend as on the map. Category values become the legend labels verbatim, so emit readable names from the source SQL rather than raw codes.
