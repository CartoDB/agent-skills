...
- **Aggregated columns on h3 / quadbin / heatmapTile / clusterTile layers** — reference `{{column_aggregation}}`, not the bare column. A `colorField: "net_worth"` with `spatialIndexAggregation: "average"` becomes `{{net_worth_average}}` in the template. Bare `{{net_worth}}` renders empty on aggregated layers.
