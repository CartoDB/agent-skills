...
- **Description — viewer-facing Markdown, OPTIONAL.** Empty/omitted descriptions don't render at all for the viewer (the right-rail info button is hidden when description is empty), so only emit one when it adds value — a takeaway, a caveat, or an interaction hint. When emitted, lead with the takeaway, not a layer recap — the legend and layer panel already show *what's on the map*. See `references/cartography.md` §6.4 for the template (lead paragraph → optional `## What you're seeing` → optional `## How to read it`; no `## Source` section — connection/table identifiers are author plumbing), the no-tables / 5-line cap rules, and worked examples. Not for authoring notes, agent reasoning, or change history. When you choose to emit nothing, set `description: ""` (empty string, never null — null leaks a placeholder); `maps create` auto-fills `""`, `maps update` needs an explicit `""` to clear.
